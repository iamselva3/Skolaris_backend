...
```bash
git clone <repo> && cd skolaris-backend
cp .env.example .env       # set secrets, then:
npm install
docker compose up -d redis  # plus minio/mail profiles as needed
npm run dev
```

`npm run dev` runs `scripts/dev.mjs` — a small cross-platform orchestrator that reads `.env`, decides which services your current config needs, and spawns them with prefixed/coloured logs:

| Service | Always-on? | Skipped when |
| --- | --- | --- |
| **NestJS API** (`start:dev`, port `PORT` default 3000) | yes | — |
| **OCR worker** (`ocr:real` — drains `ocr.extract`) | yes | `WORKER_MODE=both` (API runs the consumer in-process) |
| **Python OCR service** (`uvicorn` from `ocr-handwriting/.venv`) | no | `HANDWRITING_OCR_ENABLED=false` AND `PRINTED_OCR_VIA_PADDLE=false` |

It prints a startup summary (✓/✗ per service), TCP-probes Redis up front (aborts with a `docker compose up -d redis` hint if it's down), and traps Ctrl+C to shut every child down — `taskkill /t` on Windows so grandchildren (ts-node → tesseract) don't leak. The Python venv is detected at `ocr-handwriting/.venv/` (or `venv/`); if it's missing while a Python-needing flag is on, the orchestrator prints the exact `python -m venv && pip install` command to set it up and exits.

The existing scripts are **kept untouched** for compat — `npm run dev:full`, `npm run dev:full:mock`, `npm run ocr:real`, `npm run start:dev`, etc. all behave exactly as before. `npm run dev` is the new canonical entry point; the others are escape hatches.

Production (Render, `start:prod`, the `WORKER_MODE` topology) is **unaffected** — `scripts/dev.mjs` only runs when you invoke it explicitly via `npm run dev`.
...
```
cd existing_repo
git remote add origin https://gitlab.com/skolaris/Skolaris-backend.git
git branch -M main
...
* [Set up project integrations](https://gitlab.com/skolaris/Skolaris-backend/-/settings/integrations)
...
>>>>>>> c38d8af4bf892cc25893ffb794b730ee4ab4e084
